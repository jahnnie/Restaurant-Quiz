...
public class ScoreTracker {
    ...
    private int findHighestPointsEarned() {
        ...
        for (int i = 0; i < scores.size(); i++) {
            if(scores.get(i).getPointsEarned() > highScore){
                highScore = scores.get(i).getPointsEarned();
            }

        }
        return highScore;   // stub
        */
    ...
    private int findMinAttemptsTaken() {
        /*
        int lowestAttempt = Integer.MAX_VALUE;

        for (int i = 0; i < scores.size(); i++) {
            if(scores.get(i).getNumAttempts() < lowestAttempt)  {
                lowestAttempt = scores.get(i).getNumAttempts();
            }
        }

        return lowestAttempt;  // stu
        */
